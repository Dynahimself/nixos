...
let cfg = config.dyna.nixvim; in
{
  ...
  programs.nixvim = {
    ...
    keymaps = [
      ...
      # `i` = up in NORMAL mode ONLY — in visual and operator-pending, `i` keeps
      # its builtin meaning (inner text objects: ci{, ciw, di(, viw, vit...).
      # Same principle as stock vim: `a` = append (normal) but around (aw, a{).
      { mode = "n"; key = "n"; action = "h"; options.desc = "Left"; }
      { mode = [ "v" "x" ]; key = "n"; action = "h"; options.desc = "Left"; }
      { mode = "n"; key = "e"; action = "gj"; options.desc = "Down (wrapped)"; }
      ...
      { mode = "o"; key = "j"; action = "e"; options.desc = "To end of word"; }
      # k/K = search next/prev (formerly n/N) — normal mode only;
      # visual needs k for up since i is the text-object prefix there
      { mode = "n"; key = "k"; action = "n"; options.desc = "Search next"; }
      { mode = "n"; key = "K"; action = "N"; options.desc = "Search previous"; }
      { mode = [ "v" "x" ]; key = "k"; action = "gk"; options.desc = "Up (wrapped)"; }
      # gl = insert at last insert position (builtin gi is unreachable: i is shadowed)
      { mode = "n"; key = "gl"; action = "gi"; options.desc = "Insert at last position"; }
